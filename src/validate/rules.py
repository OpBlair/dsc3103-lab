...
def rule_missing_market(df):
    ...
    missing_mark = (df["market"].isnull()) | (df["market"] == "")
...
def rule_known_commodity(df):
    # valid commodities
    known_commodities = ["beans", "maize"]

    # find rows where commodity is not in the known list
    unknown_mask = ~df["commodity"].str.strip().str.lower().isin(known_commodities)
...
def rule_negative_rain(df):
    neg_rainfall = df[df["rainfall_mm"] < 0].copy()
    neg_rainfall["Reason"]="Negative Rainfall"
    return neg_rainfall
